...
BOOL CImgPgm::SaveFile( CString *file_name, CString *dir_name )
{
int		x, y;
FILE *fp;

	if( (fp=GetSaveFilePointer( file_name, dir_name )) == NULL ) return FALSE;
...
//	double theta=0.3; /* 回転角度 */
	...
	switch( kind ) {
		case 1:
				...
				break;
		case 2: // 周期インパルス
				if( theta <= 0 ) theta = 0.0001; //00001;
				if( theta > epi/2 )  
				{
					theta = 1.57;
				 	msg.Format( "最大回転角度1.57ラジアンです" );
				AfxMessageBox( msg );
				}
