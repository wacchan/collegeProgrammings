...
public class SmtpClient_kadai4 {
	static InputStreamReader isr2 = new InputStreamReader(System.in); 
	static BufferedReader br = new BufferedReader(isr2);
    ...
    static final String subjectText="発展課題";  //件名（Subject）
    ...
    public static void sendHeader() throws IOException {
		...
		//ヘッダーSubject
		System.out.println("subjectを入力してください");
		//byte[] sText = subjectText.getBytes("ISO2022JP");        //subjectTextのバイト化
		//byte[] sText2 = br.readLine().getBytes("ISO2022JP");     //キーボードからのsubject入力、バイト化
        //byte[] sText3 = new byte[sText.length + sText2.length];
        
		//System.arraycopy(sText, 0, sText3, 0, sText.length);
        //System.arraycopy(sText2, 0, sText3, sText.length, sText2.length);//二つの連結
        
		String encoded = Base64.getEncoder().encodeToString(br.readLine().getBytes("ISO2022JP"));//課題2と同様
        dataString = "Subject: " + "=?iso-2022-jp?B?" + encoded + "?=";//同上
        
		writeSomething(dataString);
		System.out.println("　　　　　  - " + dataString);

		dataString = "MIME-Version: 1.0"; // 課題3で変更
		writeSomething(dataString);
		System.out.println("　　　　　  - " + dataString);

		dataString = "Content-Type: text/plain; charset=ISO-2022-JP";// 課題3で変更
		writeSomething(dataString);
		System.out.println("　　　　　  - " + dataString);

		dataString = "Content-Transfer-Encoding: base7";// 課題3で変更
		writeSomething(dataString);
		System.out.println("　　　　　  - " + dataString);
